...
get_weather() {
  local location=$(get_tmux_option "@my_weather_location")
  local format=$(get_tmux_option "@my_weather_format" 1)
  local units=$(get_tmux_option "@my_weather_units" "m")

  if [ "$units" != "m" ] && [ "$units" != "u" ]; then
    units="m"
  fi

  curl -s "https://wttr.in/$location?$units&format=3" | sed "s/[[:space:]]km/km/g"
}

main() {
  local update_interval=$((60 * $(get_tmux_option "@my_weather_interval" 15)))
